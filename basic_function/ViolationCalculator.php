...
$vc = new ViolationCalculator;
$vc->calculateViolations();
...
class ViolationCalculator
{
     ...
     * @author Oleg Manov
     */ 
    public function calculateViolations()
    {
        $options = getopt("t:d:i:c:");

        if (array_key_exists("d", $options) && array_key_exists("i", $options) && array_key_exists("t", $options))
        {
            $startDate  = $options['t'];
            $driverId   = $options['i'];
            $company    = $options['d'];
            ...
            if (!is_null($clientId))
            {
                ...
                {
                    $startDate = (new \DateTime($startDate))->modify('-2 months');
                    foreach ($allDrivers as $driver)
                    {
                        $driver = $this->dbQuery->getDriverInfo($driver['driver_idx']);
                        if (!is_null($driver))
                        {
                            $lastShift = $this->dbQuery->getLastShiftV($driver['idx'], $startDate->modify('-2 months')->format('Y-m-d'));
                        // var_dump($lastShift);
                            if (!is_null($lastShift['dateStart']))
                            {
                                $startDate = new \DateTime($lastShift['dateStart']);
                                echo GREEN . "Letzte Schicht Vio: " . $lastShift['dateStart'] . RESET . "\n";
                            }
                            $this->startCalculations($driver, $startDate->format('Y-m-d'));
                        }
                        else
                        {
                            echo RED . "Fahrer nicht gefunden! " . $driver['driver_idx'] . RESET . "\n";
                        }
                    }
                }
            }
            else
            {
                ...
                {
                    echo RED . "Fahrer nicht gefunden! " . $driverId . RESET . "\n";
                }
            }
        }
        else
    ...
    {
        print (GREEN. 'DRIVER ID: '.$driver['idx'] . RESET . "\n");
    ...
    {
        var_dump('Calculating LSV for driver #'.$driver['idx']);
        $violations = array();
        //var_dump($driverCardIds);
        //var_dump($workingTimes);
    ...
    public function calculateAbsentAdditionAndWrongActivity($driver, $driverCardIds, $startDate)
    {
        ...
        var_dump('Calculating AA and WS for driver #'.$driver['idx']);
    ...
    {
        var_dump('Calculating WDT for driver #'.$driver['idx']);
       // var_dump($driverCardIds);
        $violations = array();
        $activities = null;
           ...
           //$this->dbQuery->resetShortenedWRT($driverId, $startDate);
            ...
            foreach ($restTimes as $restTime)
            {
                ...
                if (!is_null($lastRestTime))
                {
                       ...
                       // var_dump($shortenedWRTs);
                        ...
                        else
                        {
                            ...
                            elseif (count($potentialWRTs) > 1)
                            {
                                ...
                                $keySWRT = $this->chooseShortenedWRT($driverId, $potentialWRTs, $lastRestTime['payment_end'], $restTime['payment_start'], $driverCardIds);
                    ...
                    elseif ($diff <= 8640)
                    {
                        $dailyViolations = $this->checkForDailyViolations($driverId, $lastRestTime['date_end_utc'], $restTime['date_start_utc'], $driverCardIds) ;
                        ...
                        if ($dailyViolations > 0)
                        {
                            ...
                            if (!is_null($shortenedWRTs) && count($shortenedWRTs) > 0)
                            {
                                $keySWRT = $this->chooseShortenedWRT($driverId, $shortenedWRTs, $lastRestTime['payment_end'], $restTime['payment_start'], $driverCardIds);
                                //var_dump('NEW !!!');
                                //var_dump($keySWRT);
    ...
    private function chooseShortenedWRT($driverId, $weeklyRestTimes, $startDate, $endDate, $driverCardIds)
    {
        $violations = array();
        ...
        {
            $violations[$key] = $this->checkForDailyViolations($driverId, $startDate, $weeklyRestTime['payment_end'], $driverCardIds);
            $violations[$key] += $this->checkForDailyViolations($driverId, $weeklyRestTime['payment_end'], $endDate, $driverCardIds);
        }
        //var_dump($violations);
        $bestChoise = null;
    ...
    private function checkForDailyViolations($driverId, $startDate, $endDate, $driverCardIds)
    {
        $fines = 0;

        $ddtViolations = $this->calculateDaylyDrivingTimeViolations($driverId, $startDate, $endDate, $driverCardIds);
    ...
    public function calculateDailyViolations($driverId, $driverCardIds, $startDate)
    {
        ...
        var_dump('Calculating DDT for driver #'.$driverId);
    ...
    {
        var_dump('Calculateing DB for driver #'.$driverId);
        $violations = array();
    ...
    private function getDailyBreaksViolations($driverId, $driverCardIds, $dateStart, $dateEnd, $biggestPause, $violationPause15, $drivingTime, $drivingTimeAfterBiggestPause, $drivingTimeToBiggestPause)
    {
        ...
        if ($violationStartEnd['start'] != null )
        {
    ...
    {
        var_dump('Calculateing AZG for driver #'.$driverId);
        $violations = array();
    ...
    {
        var_dump('Calculateing WLDB for driver #'.$driverId);
        $violations = array();
                                    ...
                                    else
                                    {
                                       ...
                                       // var_dump($violationWT);
                                        ...
                                        if ($activity['duration'] < 30)
                                        {
                                            ...
                                            //TODO calc date_end
                                            $violationP = array('driver_idx' => $driverId, 'violation_idx' => 19, 'date_start' => $activity['date_end_utc'],
                                                        'date_end' => $activity['date_end_utc'], 'duration' => $violationDurationP, 'group' => null, 'addition' => null, 'fine'=> $fine);
                                            //var_dump($violationP);
                                            array_push($violations, $violationP);
                                            //var_dump('FALL 7');
                                        }
    ...
    {
        var_dump('Calculateing DWT for driver #'.$driverId);
        $violations = array();
