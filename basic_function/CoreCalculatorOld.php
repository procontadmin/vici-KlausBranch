...
class CoreCalculator
{
    ...
    public function run()
    {
        ...
        elseif (array_key_exists("i", $options) && array_key_exists("d", $options))
        {
            ...
            $startDateE = "2020-01-01 00:00:00";
            $dateStartD = "2020-01-01";
            $driverId = $options['i'];
            $systemId = $options['d'];
            $this->dbQuery = new CoreDbQueries($systemId);
            ...
            if (!is_null($driver))
            {
                ...
                if (!is_null($lastDocket['dateStart']))
                        {
                            $lastDocket = $this->dbQuery->getLastDShift($driver['idx'], $lastDocket['dateStart']);
                            
                            if (!is_null($lastDocket['dateStart']))
                            {
                                $dateStartD = $lastDocket['dateStart'];
                                echo GREEN . "Letzter Lohntag: " . $dateStartD . RESET . "\n";
                            }
                            else
                            {
                                echo BLUE . "Start Date: " . $dateStartD . RESET . "\n";
                            }
                        }
                        else
                        {
