...
class DocketCalculator
{
    ...
    public function run()
    {
        ...
        if (array_key_exists("a", $options) && array_key_exists("d", $options))
        {
            ...
            if(!is_null($allDrivers))
            {
                ...
                {
                    //var_dump($driver);
                    $this->proceedDriver($driver['driver_idx'], "2020-01-01");
                }
            }
        }
        else if (array_key_exists("d", $options) && array_key_exists("i", $options))
        {
            //$startDate  = $options['t'];
    ...
    private function proceedDriver($driverId, $startDate)
    {
		...
		if ($dateStart < $driver['contract_start'])
                        $dateStart = $driver['contract_start'];
                ...
                $wageGroups = $this->dbQuery->getClientPaymentGroups($cid, 2, $driverId);
                //var_dump($wageGroups);
...
//var_dump($activities);
        ...
        if (!is_null($activities) && count($activities)>0)
        {
            ...
            $this->dbQuery->deleteWageTimes($driver['idx'], '2021-01-01');
	    if ($wageTimes != '')
