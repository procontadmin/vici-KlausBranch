...
$wtc = new AllowancesCalculator();
$wtc->calculateAllowances();
...
class AllowancesCalculator
{
    ...
    public function calculateAllowances()
    {
        ...
        if (array_key_exists("t", $options) && array_key_exists("i", $options) && array_key_exists("d", $options)) {
            $startDate  = $options['t'];
            $driverId   = $options['i'];
            $company    = $options['d'];
            ...
            if (!is_null($clientId)) {
                ...
                if(!is_null($allDrivers)) {
                    foreach ($allDrivers as $driver) {
                        $driver = $this->dbQuery->getDriverInfoAllowances($driver['driver_idx']);
                        ...
                        if (!is_null($driver)&& count($driver) > 0) {
                            ...
                            for ($i = 0; $i < count($driver); $i++) {
                                ...
                                foreach ($driverTypes as $driverType) {
                                    ...
                                    $aGroupsSC = $this->dbQuery->getClientPaymentGroupsAllowances($cid, 1, $driver[$i]['idx'], $driver[$i]['history_idx'], $driverType);
                ...
                $driver = $this->dbQuery->getDriverInfoAllowances($driverId);
                if (!is_null($driver) && count($driver) > 0) {
                    ...
                    for ($i = 0; $i < count($driver); $i++) {
                        ...
                        foreach ($driverTypes as $driverType) {
                            ...
                            //var_dump($aGroupsSC);
                            if (false && (!is_null($aGroupsHB) && count($aGroupsHB)>0) || (!is_null($aGroupsDB) && count($aGroupsDB)>0) ||
                                        (!is_null($aGroupsSB) && count($aGroupsSB)>0) || (!is_null($aGroupsSC) && count($aGroupsSC)>0)) {
                                $this->startDriverCalculations($driver[$i], $aGroupsHB, $aGroupsDB, $aGroupsSB, $aGroupsSC, $dateStart->modify('-1 day')->format('Y-m-d'));
                            } else {
                                echo RED . "Keine Zulagen gefunden!" . RESET . "\n";
    ...
    private function startDriverCalculations($driver, $aGroupsHB, $aGroupsDB, $aGroupsSB, $aGroupsSC, $startDate)
    {
        ...
        echo(GREEN. 'DRIVER ID: '.$driver['idx'] . RESET . "\n");
    ...
    private function calculateDailyBasedAllowances($driver, $startDate, $workingTimes, $allowancesGroups)
    {
        ...
        if (!is_null($driver['allowance_groups_idx'])) {
