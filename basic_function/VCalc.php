...
class VCalc
{
    ...
    public function run($sDate)
    {
        ...
        $lastShift = $this->dbQuery->getLastShiftV($this->driver['idx'], $startDate->modify('-4 months')->format('Y-m-d'));
        // $lastShift = $this->dbQuery->getLastShiftV($this->driver['idx'], $startDate->format('Y-m-d'));
        ...
        echo GREEN . "Letzte Schicht Vio: " . $startDate->modify('-4 months')->format('Y-m-d') . RESET . "\n";
    ...
    {
        print('Calculating LandSignViolation for driver #'.$driver['idx']."\n");
        $violations = array();
    ...
    public function calculateAbsentAdditionAndWrongActivity($driver, $driverCardIds, $startDate)
    {
        ...
        print('Calculating AbsentAddition and WrongActivity for driver #'.$driver['idx']."\n");
        print('starting with '.$startDate."\n");
    ...
    {
        print('Calculating WeeklyDrivingTime for driver #'.$driver['idx']."\n");
        print('starting with '.$startDate."\n");
        // var_dump($driverCardIds);
        $violations = array();
        $activities = null;
    ...
    public function calculateWeeklyRestTimes($driverId, $driverCardIds, $startDate)
    {
        ...
        print('Calculating WeeklyRestTime for driver #'.$driverId."\n");
        print('starting with '.$startDate."\n");
    ...
    public function calculateDailyViolations($driverId, $driverCardIds, $startDate)
    {
        ...
        print('Calculating DailyDrivingRestTimes for driver #'.$driverId."\n");
        print('starting with '.$startDate."\n");
        ...
        if (!is_null($weeklyRestTimes) && count($weeklyRestTimes) > 0) {
            ...
            foreach ($weeklyRestTimes as $weeklyRestTime) {
                ...
                //echo($weeklyRestTime['payment_end']."\n");
    ...
    {
        print('Calculating DailyBreaks for driver #'.$driverId."\n");
        $violations = array();
    ...
    {
        print('Calculating WeeklyWorkingTimes for driver #'.$driverId."\n");
        $violations = array();
    ...
    {
        print('Calculating WeeklyDailyBreaks for driver #'.$driverId."\n");
        $violations = array();
                        ...
                        if ($activity['activity'] == 0) {
                            ...
                            if ($activity['duration'] >= 15) {
                                ...
                                if ($block6h > 360 && $block6h != $workingTime) {
                                    ...
                                    $fine = intval(ceil($violationDuration/30)*80);
                                ...
                                if  ($workingTime > 540 && $durationPause < 45) {
                                    if ($durationPause > 0) {
                                        ...
                                        $fineWT = intval(ceil($violationDurationWT/30)*80);
                                        ...
                                        $fineP = intval(ceil($violationDurationP/15)*80);
                                        //TODO calc date_end
    ...
    {
        print('Calculating DailyWeeklyTimes for driver #'.$driverId."\n");
        $violations = array();
