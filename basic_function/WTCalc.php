...
class WTCalc
{
    ...
    public function run($startDate)
    {
        ...
        $drivers = $this->dbQuery->getDriverInfoAllowances($this->driver["idx"]);
        //var_dump($driver);
        ...
        if (!is_null($drivers))
        {
            ...
            {
                $this->proceedDriver($driver, $startDate);
            }
        }
    ...
    {
        echo (SKY . 'Fahrer ID: ' . $driver['idx'] . RESET . "\n");
    ...
    private function proceedWorkingTimes($driver, $startDate, $workingTimes)
    {
        ...
        if ($driver['client_idx'] == 33 || $driver['client_idx'] == 69)
        {
